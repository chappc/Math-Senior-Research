...
\begin{dmath*}
\vec{l} = position of light\\
\vec{r} = the radius of each sphere and cylinder\\
...
\begin{algorithm}[H]
\DontPrintSemicolon
 \KwData{$\vec{l}$ = position of light, $\vec{r}$ = the radius of each sphere and cylinder, $spheres$ = a list of positions of centers of spheres, $cylinders$ = a list of pairs of positions specifying the end faces of the cylinders}
 \KwResult{$pixels$ = array of black and white pixels indicating on which areas of the wall light is incident}
 $pixels \leftarrow$ a uniformly white (illuminated) image of the desired size\;
 ...
 \For{each cylinder in cylinders}{
  \tcp*[h]{similar to the above but cylinders cast different}
  \tcp*[h]{shaped shadows}\\
  \tcp*[h]{This case is not handled by our code.}
 }
 \caption{first shadow rendering method}
\end{algorithm}
